...
int urlparse(const char *url, bool *is_https, char *host, char *port, char *path)
{
    ...
    if (!sscanf(url, "%" URLLEN_S "[^:/]", host))
        return -1;
    url += strlen(host);

    if (*url == ':') {
        if (!sscanf(url + 1, "%" URLLEN_S "[0123456789]", port))
            return -1;
        url += strlen(port) + 1;
    } else
...
FILE *urlopen(char *url, unsigned flags, const char *accept)
{
    ...
    if (urlparse(url, &is_https, host, port, path) < 0)
        goto out;
    ...
    while (fgets(buf, 1024, f) && buf[0] != '\r') {
        ...
        if (sscanf(buf, "content-encoding: %1000s", header)) {
            if (!strcmp(header, "gzip"))
                inflate = 1;
            else if (!strcmp(header, "deflate"))
                inflate = -1;
            else {
    ...
    if (length)
        f = ftrunc(f, length, TRUNC_AUTOCLOSE);
    if (chunked)
        f = fchunk(f, CHUNK_AUTOCLOSE);
    if (inflate == 1)
        f = finfl(f, INFL_AUTOCLOSE);
    if (inflate == -1)
        f = finfl(f, INFL_RAW | INFL_AUTOCLOSE);
