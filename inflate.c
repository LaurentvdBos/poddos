...
static ssize_t zread(void *cookie, char *buf, size_t n)
{
    ...
    do {
        ...
        if (ret == Z_STREAM_END && strm->avail_in) fprintf(stderr, "zread: end of stream with bytes pending.\n");
...
static int zclose(void *cookie)
{
    ...
    z_stream *strm = &z->strm;
    inflateEnd(strm);
    if (z->flags | INFL_AUTOCLOSE) ret = fclose(z->f) ? -1 : 0;
    free(z);
    return ret;
}
