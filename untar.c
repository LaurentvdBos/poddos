...
void tarwrite(struct tarfile file, FILE *f, int dir_fd)
{
    ...
    case '7':
        ...
        if (fd == -1) err(1, "open(%s)", file.path);

        char buf[512];
        int n;
        while ((n = fread(buf, 1, 512, f)) > 0) {
            if (write(fd, buf, n) == -1) err(1, "write(%s)", file.path);
        }
        if (n == -1) err(1, "fread");

        if (fchown(fd, file.uid, file.gid) == -1) err(1, "fchown(%s, %d, %d)", file.path, file.uid, file.gid);
        if (fchmod(fd, file.mode)) err(1, "fchmod(%s, 0%03o)", file.path, file.mode);

        const struct timeval tvp[] = { file.mtime, file.atime };
        if (futimes(fd, tvp)) err(1, "futimes(%s)", file.path);

        close(fd);
        break;
    
    case '1':
        if (linkat(dir_fd, file.linkpath, dir_fd, file.path, 0) == -1) err(1, "linkat(%s, %s)", file.linkpath, file.path);
        break;
    
    case '2':
        if (symlinkat(file.linkpath, dir_fd, file.path) == -1) err(1, "symlinkat(%s, %s)", file.linkpath, file.path);
        break;
    
    case '3':
        if (mknodat(dir_fd, file.path, file.mode, makedev(file.major, file.minor) | S_IFCHR) == -1) err(1, "mknodat(%s)", file.path);
        if (fchmodat(dir_fd, file.path, file.mode, 0) == -1) err(1, "chmod(%s)", file.path);
        break;
    
    case '4':
        if (mknodat(dir_fd, file.path, file.mode, makedev(file.major, file.minor) | S_IFBLK) == -1) err(1, "mknodat(%s)", file.path);
        if (fchmodat(dir_fd, file.path, file.mode, 0) == -1) err(1, "chmod(%s)", file.path);
        break;
    
    case '5':
        if (mkdirat(dir_fd, file.path, 0777) == -1 && errno != EEXIST) err(1, "mkdir(%s)", file.path);
        if (fchmodat(dir_fd, file.path, file.mode, 0) == -1) err(1, "fchmod(%s, 0%03o)", file.path, file.mode);
        break;
    
    default:
        errx(1, "Unrecognized type: %c\n", file.type);
    }
}
...
unsigned unpax(FILE *f, struct tarfile *file)
{
    ...
    while (!ferror(f) && !feof(f)) {
        ...
        for (;;) {
            ...
            n++;
            if (c == ' ') break;
            else if ('0' <= c && c <= '9') len = 10*len + (c - '0');
            else return flags; // Ignore invalid pax headers
        }

        if (len > PAX_MAX) errx(1, "Pax header too long: %d", len);

        // Read the rest
        char key[PAX_MAX];
        if (fread(key, 1, len - n, f) < len - n) return flags;
        ...
        *(val - 1) = 0;
        ...
        if (!strcmp(key, "atime")) {
            double v;
            if (!sscanf(val, "%lf", &v)) return flags;
            file->atime.tv_sec = v;
            ...
            if (!sscanf(val, "%d", &file->uid)) return 0;
            flags |= PAX_UID;
        }
        else if (!strcmp(key, "gid")) {
            if (!sscanf(val, "%d", &file->gid)) return 0;
            flags |= PAX_GID;
        }
        ...
        else warnx("Unrecognized pax key in %s: %s", file->path, key);
    }
...
FILE *untar(FILE *f, struct tarfile *file)
{
    ...
    while (!ferror(f) && !feof(f)) {
        ...
        if (blksize % 512) blksize += 512 - (blksize % 512);

        if (strncmp(tar->ustar, "ustar", 5)) return NULL;
        ...
        switch (tar->type) {
            ...
            case 'L':
                ...
                paxflags |= PAX_PATH;
                if (fread(file->path, 1, blksize, f) != blksize) return NULL;
                continue;
            
            case 'K':
                // Same as 'L', but then for the linkpath
                paxflags |= PAX_LINKPATH;
                if (fread(file->linkpath, 1, blksize, f) != blksize) return NULL;
                continue;
        }
